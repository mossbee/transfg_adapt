...
class NDTWIN(Dataset):
    def __init__(self, root, pair_txt=None, transform=None):
        self.root = root
        ...
        with open(pair_txt, 'r') as f:
            for line in f:
                img1_name, img2_name, label = line.strip().split()
                self.pairs.append((img1_name, img2_name, int(label)))

    def load_image(self, img_name):
        if img_name in self.cache:
            return self.cache[img_name]
        img_path = os.path.join(self.root, img_name[:5], img_name)
        img = Image.open(img_path).convert('RGB')
        self.cache[img_name] = img
        return img

    def __getitem__(self, index):
        img1_name, img2_name, label = self.pairs[index]

        img1 = self.load_image(img1_name)
        img2 = self.load_image(img2_name)
