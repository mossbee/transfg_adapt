...
def valid_verification(args, model, writer, test_loader, global_step):
    ...
    accuracy = simple_accuracy(preds.numpy(), all_labels.numpy())
    accuracy = torch.tensor(accuracy).to(args.device)
    dist.barrier()
    val_accuracy = reduce_mean(accuracy, args.nprocs)
    val_accuracy = val_accuracy.detach().cpu().numpy()
...
def main():
    ...
    parser.add_argument("--dataset", choices=["NDTWIN","CUB_200_2011", "car", "dog", "nabirds", "INat2017"], default="NDTWIN",
                        help="Which dataset.")
    parser.add_argument('--data_root', type=str, default='/home/mossbee/Work/Dataset/ND_TWIN_Dataset_Frontal_448')
